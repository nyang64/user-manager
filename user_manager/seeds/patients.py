...
from seeds.seed_data import PATIENTS
...
def create_patient(patient_details, outpatient_provider, prescribing_provider):
    ...
    patient = patient_schema.load(
        {
            ...
            "indication": patient_details["indication"],
    ...
    message_details[
        "patient"
    ] += f"Patient with id '{patient.id}' created with user id {registered_user_id}. "
...
def create_patient_device(patient_id, serial_number):
    ...
    message_details[
        "patient"
    ] += f"Patient with id '{patient_id}' was associated with device serial number {serial_number}. "
...
def create_patient_provider(patient_id, provider, role_id):
    ...
    message_details[
        "patient"
    ] += f"Patient with id '{patient_id}' was associated with provider id {provider.id}. "
    print_message_details()
