...
class PatientServices(DbRepository):
    ...
    def count_device_assigned(self, patient_id):
        ...
        try:
            ...
            for device in devices:
                ...
                resp = requests.get(
                    GET_DEVICE_STATUS_URL, headers=header, params=payload
                )
                logging.info(
                    "Request finished with status code {}".format(resp.status_code)
                )
    ...
    def check_device_assigned(self, device_serial_number):
        is_assign = PatientsDevices.check_device_assigned(device_serial_number)
        print("Device Assigned to patient" + str(is_assign))
        logging.debug("Device Assigned to patient : {}".format(bool(is_assign)))
        if bool(is_assign) is True:
            logging.warning(
                "Device {} Already assigned to patient".format(device_serial_number)
            )
    ...
    def check_device_number_exist(self, device_serial_number):
        ...
        r = requests.get(CHECK_DEVICE_EXIST_URL, headers=header, params=payload)

        logging.debug("Request finished with status code {}".format(r.status_code))
        logging.debug("response {}".format(r.text))
        logging.debug("The Called API {}".format(r.url))
        if int(r.status_code) == 404:
            MSG = "Device serial number {} not found".format(device_serial_number)
            raise NotFound(MSG)
    ...
    def update_device_status(self, device_serial_number):
        ...
        payload = {"serial_number": device_serial_number, "name": DEVICE_STATUS}
        logging.info("Request payload {}".format(payload))
        resp = requests.post(UPDATE_DEVICE_STATUS_URL, headers=header, json=payload)
        logging.info("Request finished with status code {}".format(resp.status_code))
        logging.info("response {}".format(resp.text))
        logging.info("The Called API {}".format(resp.url))
        if int(resp.status_code) != 201:
            logging.warning("Failed to update the device status")
            raise InternalServerError("Failed to update status. Device Not Assigned")
        else:
            return True
    ...
    def assign_device_to_patient(self, patient_device):
        ...
        try:
            ...
            updated = self.update_device_status(patient_device.device_serial_number)
            if updated is True:
                self.commit_db()
                return patient_device
        except SQLAlchemyError as error:
            logging.error(
                "Error Occured while assign device to patient {}".format(str(error))
            )
            raise InternalServerError(str(error))
    ...
    def patient_device_list(self, token):
        ...
        for d in device_serial_numbers:
            ...
            r = requests.get(GET_DEVICE_DETAIL_URL, headers=header, params=payload)
