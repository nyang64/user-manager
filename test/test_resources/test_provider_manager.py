from resources.provider_manager import provider_manager as ProviderManager
...
class TestProviderManager(TestCase):
    ...
    def test_get_report_signed_link(self, request, mock_patient):
        ...
        with app.test_request_context():
            resp = ProviderManager.get_report_signed_link.__wrapped__(
                self.provider, '')
    ...
    def test_update_uploaded_ts(self, request, mock_patient):
        ...
        mock_patient.return_value = 'signed', http.client.OK
        with app.test_request_context():
            resp = ProviderManager.update_uploaded_ts.__wrapped__(
                self.provider, '')
            self.assertEqual(resp[0], {'message': 'signed',
    ...
    def test_add_facility(self, request, mock_patient):
        ...
        expected = {'address_id': 1,
                    'facility_id': 2,
                    'status_code': http.client.CREATED}
        with app.test_request_context():
            resp = ProviderManager.add_facility.__wrapped__(
                self.provider, '')
